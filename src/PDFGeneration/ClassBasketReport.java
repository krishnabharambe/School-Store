 ...
 * @author admin
 */
public class ClassBasketReport {
    ...
    public ClassBasketReport(String yearsession, String stdsection) {
        final String Destination = getpath(yearsession);
        File file = new File(Destination);
        file.getParentFile().mkdirs();
        createPdf(Destination, yearsession, stdsection);
...
//        createPdf("Report/"+Vquotationno + "-" + timestamp + ".pdf");
    ...
    private String getpath(String yearsession) {
        ...
        return desktopPath.replace("\\", "/") + "/Dpark/" + yearsession + "/Class Report Report/" + datenew + bpath1 + ".pdf";
    }
    ...
    private void createPdf(String Destination, String yearsession, String stdsession) {
        try {
            // step 1
            ...
            PdfPCell p5 = new PdfPCell(new Paragraph("BASKET SALES REPORT OF "+stdsession, H14B));
            p5.setHorizontalAlignment(Element.ALIGN_CENTER);
            p5.setBorder(0);
            t21.addCell(p5);
            document.add(t21);
            
            ResultSet RS = getSales(yearsession, stdsession);
...
//            ResultSet RS = db.getdata("Select stdsection from sales2 group by stdsection order by stdsection");
            ...
            int i = 1;
            while (RS.next()) {
                float[] columnWidths1 = {3, 20,5};
                pTable = new PdfPTable(columnWidths1);
                pTable.addCell(new Paragraph(String.valueOf(i)));
                pTable.addCell(new Paragraph(RS.getString("studentname")));
                pTable.addCell(new Paragraph(RS.getString("dated_on")));
                document.add(pTable);
                i++;
            }
...
//            
            ...
            Logger.getLogger(ClassBasketReport.class.getName()).log(Level.SEVERE, null, ex);
