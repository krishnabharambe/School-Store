...
import Database.db;
import static DbController.IndiSalesConstroller.getValueFromindiSalesId;
...
public class PDFAllCLassindi {
    ...
    public PDFAllCLassindi(String yearsession, String fromdate, String todate) {
...
//        createPdf("Report/"+Vquotationno + "-" + timestamp + ".pdf");
    ...
    private void createPdf(String Destination, String yearsession, String fromdate, String todate) {
        try {
            ...
            p1.setBorder(0);
            t1.addCell(p1);
            PdfPCell p2 = new PdfPCell(new Paragraph("REPORT", H14B));
            ...
            PdfPCell p5 = new PdfPCell(new Paragraph("INDIVIUAL SALES REPORT", H14B));
            p5.setHorizontalAlignment(Element.ALIGN_CENTER);
            p5.setBorder(0);
            t21.addCell(p5);
            document.add(t21);

            ResultSet RS = db.getdata("Select stdsection from indisales2 group by stdsection order by stdsection");
            ...
            while (RS.next()) {
                ...
                document.add(p13);
                ResultSet data = db.getdata("Select * from indisales2 WHERE yearsessionName = '" + yearsession + "' AND  stdsection = '" + stdsession + "'  AND dated_on BETWEEN '" + fromdate + "' AND '" + todate + "'");

//                price = getbasketvalue(yearsession, stdsession);
//price = 2000;
                float[] columnWidths1 = {15, 4};
                float internal = 0;
                while (data.next()) {

                    pTable = new PdfPTable(columnWidths1);
                    pTable.addCell(new Paragraph(data.getString("studentname"), H14));

                    pTable.addCell(new Paragraph("Rs." + getValueFromindiSalesId(data.getString("saleid")), H14));
                    internal = internal + getValueFromindiSalesId(data.getString("saleid"));
                    document.add(pTable);

                }

                PdfPTable p2Table = new PdfPTable(columnWidths1);
                p2Table.addCell(new Paragraph("Total Of " + RS.getString("stdsection"), H14B));
                p2Table.addCell(new Paragraph("Rs." + internal, H14B));
            ...
            pTable13.addCell(new Paragraph("Total  : ",H16B));
            pTable13.addCell(new Paragraph("Rs " + external,H16B));
